...
def common_generate(args: argparse.ArgumentParser, bfnType: BFNType, timeType: TimeType):
    ...
    with torch.no_grad():
        x_hat, out_list = bfn.sample(args.height, device=device, steps=args.step, return_samples=return_samples, batch_size=args.batch, ema=ema)
    print(x_hat)
    # if args.gif_generate:
...
def setup_generate_common_parser(parser: argparse.ArgumentParser) -> argparse.ArgumentParser:
    parser.add_argument("--load_model_path", type=str, required=True, default="./models/model_40.pth", help="Path to the pre-trained model file.")
    ...
    parser.add_argument("--height", type=int, default=256, help="Height of the generated images.")
    ...
    parser.add_argument("--generate_input_output_image", default=False, action="store_true", help="Whether to generate input(θ, mu, ...) and output images.")
    parser.add_argument("-gif", "--gif_generate", default=False, action="store_true", help="Whether to generate output images gif.")
...
common_generate(args, bfnType=BFNType.Continuous, timeType=TimeType.ContinuousTimeLoss)