...
class ResidualBlock(nn.Module):
    def __init__(self, in_channels, out_channels, n_time=1280):
        ...
        self.conv_feature = nn.Conv1d(in_channels, out_channels, kernel_size=3, padding=1)
        self.linear_time = nn.Linear(n_time, out_channels)

        self.groupnorm_merged = nn.GroupNorm(32, out_channels)
        self.conv_merged = nn.Conv1d(out_channels, out_channels, kernel_size=3, padding=1)
    ...
    def forward(self, feature, time):
        ...
        # print(f'time.unsqueeze(-1).unsqueeze(-1): {time.unsqueeze(-1).unsqueeze(-1).shape}')
        # print(f'time.unsqueeze(-1).unsqueeze(-1): {time.unsqueeze(-1).shape}')
        # print(f'feature: {feature.shape}')
        

        merged = feature + time.unsqueeze(-1)
...
class UNet(nn.Module):
    def __init__(self, in_channels, out_channels):
        ...
        self.encoders = nn.ModuleList([
            Switch(nn.Conv1d(in_channels, 160, kernel_size=3, padding=1)),                # input (160, 64, 64)
            Switch(ResidualBlock(160, 160), AttentionBlock(8, 20)),             # input (160, 64, 64)
            Switch(ResidualBlock(160, 160), AttentionBlock(8, 20)),             # input (160, 64, 64)
            Switch(nn.Conv1d(160, 160, kernel_size=3, stride=2, padding=1)),    # input (160, 32, 32)
            ...
            Switch(nn.Conv1d(320, 320, kernel_size=3, stride=2, padding=1)),    # input (320, 32, 32)
            ...
            # Switch(nn.Conv1d(1280, 1280, kernel_size=3, stride=2, padding=1)),  # input (1280, 16, 16)
...
class UNet_conditional(UNet):
    def __init__(self, in_channels, out_channels, logp=None):
