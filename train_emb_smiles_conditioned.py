...
def train(args: argparse.ArgumentParser, bfnType: BFNType, timeType: TimeType):
    ...
    for epoch in range(1, args.epoch+1):
        losses = []
        for X, logp in tqdm(train_loader, desc='Epoch {}'.format(epoch), unit='batch'):

            optimizer.zero_grad()
            if timeType == TimeType.ContinuousTimeLoss:
                loss = bfn.process_infinity(X.to(device), logp.to(device))
        ...
        torch.save(unet.state_dict(), f'models_conditioned/last_model.pth')
        torch.save(optimizer.state_dict(), f'models_conditioned/optimizer_state.pth')
        
        if epoch % args.save_every_n_epoch == 0:
            torch.save(unet.state_dict(), f'models_conditioned/model_{epoch}.pth')
            tqdm.write('Epoch {}: saved: {}'.format(epoch, f'models_conditioned/model_{epoch}.pth'))
...
def setup_train_common_parser(parser: argparse.ArgumentParser) -> argparse.ArgumentParser:
    parser.add_argument("--load_model_path", type=str, default='models_all_data/model_17.pth')
    parser.add_argument("--load_optimizer_path", type=str, default=None)
    parser.add_argument("--save_model_path", type=str, default="models_conditioned/model.pth")
